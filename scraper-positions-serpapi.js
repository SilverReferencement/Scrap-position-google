...
async function main() {
    ...
    if (!graphSheet) {
        ...
        await graphSheet.loadCells('A1:F1');
        ...
        graphSheet.getCell(0, 3).value = 'Moyenne = 1.5';
        graphSheet.getCell(0, 4).value = 'Moyenne = 2';
        graphSheet.getCell(0, 5).value = 'Moyenne = 3';
    ...
    const average1 = totalItems * 1; // Moyenne position 1 = 315 (Rouge - très difficile)
    const average1_5 = totalItems * 1.5; // Moyenne position 1.5 = 472.5 (Orange - difficile)
    const average2 = totalItems * 2; // Moyenne position 2 = 630 (Jaune - moyen)
    const average3 = totalItems * 3; // Moyenne position 3 = 945 (Vert - facile)

    console.log(`   → Moyenne = 1 (rouge): ${average1}`);
    console.log(`   → Moyenne = 1.5 (orange): ${average1_5}`);
    console.log(`   → Moyenne = 2 (jaune): ${average2}`);
    console.log(`   → Moyenne = 3 (vert): ${average3}`);
    ...
    for (let i = 0; i < sortedDates.length; i++) {
        ...
        // Écrire la date, la somme et les 4 moyennes
        graphSheet.getCell(rowIndex, 0).value = date;
        graphSheet.getCell(rowIndex, 1).value = data.sum;
        graphSheet.getCell(rowIndex, 2).value = average1; // Colonne C : Moyenne = 1 (rouge - très difficile)
        graphSheet.getCell(rowIndex, 3).value = average1_5; // Colonne D : Moyenne = 1.5 (orange - difficile)
        graphSheet.getCell(rowIndex, 4).value = average2; // Colonne E : Moyenne = 2 (jaune - moyen)
        graphSheet.getCell(rowIndex, 5).value = average3; // Colonne F : Moyenne = 3 (vert - facile)
    }
